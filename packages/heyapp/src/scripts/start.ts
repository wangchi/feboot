...
export default (heyappConfig: HeyappConfig): void => {
  const config = new Config();
  const { port = 3000, open = false, chainWebpack, proxy = {} } = heyappConfig;

  // ----- base start
  const name = 'main';
  const base = heyappConfig.base || '/';
  let entry = heyappConfig.entry || 'src/index.js';
  const outputPath = heyappConfig.outputPath || 'dist';
  const publicPath = heyappConfig.publicPath || '/';
  ...
  config
    ...
    .filename(
      `js/${heyappConfig.hash ? '[name].[hash:8]' : '[name]'}.bundle.js`
    )
  ...
  // chains start
  alias({ config, heyappConfig });
  babelLoader({ config });
  cssLoaders({ config, heyappConfig });
  htmlWebpackPlugin({ config });
  // chains end

  // register presets start
  try {
    registerPresets({ chainConfig: config, heyappConfig });
  } catch (e) {
    logError(e.message);
    process.exit(1);
  }
